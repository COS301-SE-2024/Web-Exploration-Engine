    ...
    if (id) {
      ...
      if (resultsData) {
        ...
        if ('errorStatus' in resultsData.robots) {
          ...
          setWebsiteStatus(resultsData.domainStatus);
          if (isMetadata(resultsData.metadata)) {
            setSummaryInfo({
          ...
          setIndustryClassification(resultsData.industryClassification ? resultsData.industryClassification.zeroShotMetaDataClassify : []);
          setDomainClassification(resultsData.industryClassification ? resultsData.industryClassification.zeroShotDomainClassify : []);
          
          // screen shot
          const screenShotBuffer = Buffer.from(resultsData.screenshot, 'base64');
          const screenShotUrl = `data:image/png;base64,${screenShotBuffer.toString('base64')}`;
          setHomePageScreenShot(screenShotUrl);

          setAddresses(resultsData.addresses);
          setEmails(resultsData.contactInfo.emails);
          setPhones(resultsData.contactInfo.phones);
          setSocialLinks(resultsData.contactInfo.socialLinks);
          setTitleTagAnalysis(resultsData.seoAnalysis.titleTagsAnalysis);
          setHeadingAnalysis(resultsData.seoAnalysis.headingAnalysis);
          setImageAnalysis(resultsData.seoAnalysis.imageAnalysis);
          setInternalLinkingAnalysis(resultsData.seoAnalysis.internalLinksAnalysis);
          setMetaDescriptionAnalysis(resultsData.seoAnalysis.metaDescriptionAnalysis);
          setUniqueContentAnalysis(resultsData.seoAnalysis.uniqueContentAnalysis);
          setSentimentAnalysis(resultsData.sentiment);
          setLightHouseAnalysis(resultsData.seoAnalysis.lighthouseAnalysis);
          setSiteSpeedAnalysis(resultsData.seoAnalysis.siteSpeedAnalysis);
          setMobileFriendlinesAnalysis(resultsData.seoAnalysis.mobileFriendlinessAnalysis);
          setXmlSitemapAnalysis(resultsData.seoAnalysis.XMLSitemapAnalysis);
          setCanonicalTagAnalysis(resultsData.seoAnalysis.canonicalTagAnalysis);
          setIndexibilityAnalysis(resultsData.seoAnalysis.indexabilityAnalysis);
          setStructuredDataAnalysis(resultsData.seoAnalysis.structuredDataAnalysis);
        }

        setScrapeNews(resultsData.scrapeNews);
