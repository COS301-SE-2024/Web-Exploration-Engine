...
export class SeoAnalysisService {
  ...
  async analyzeMetaDescription(htmlContent: string, url: string) {
    ...
    const isUrlWordsInDescription = this.areUrlWordsInDescription(
      urlWords,
      metaDescription
    );
    ...
    if (!isUrlWordsInDescription) {
      recommendations += `The words from the URL (${urlWords.join(
        ', '
      )}) aren't included in the meta description. Including these can help search engines better understand the relevance of your page. `;
    } else {
  ...
  async analyzeHeadings(htmlContent: string) {
    ...
    if (count > 0) {
      ...
      recommendations =
        'No headings (H1-H6) found. Consider adding headings to improve content structure and SEO.';
    }
  ...
  async analyzeImageOptimization(url: string, browser: puppeteer.Browser) {
    ...
    try {
      ...
      for (const img of images) {
        ...
        try {
          ...
          if (!optimized) {
            ...
            errorUrls.push(
              `Error optimizing image: ${img.src}. ${reasons.join(', ')}`
            );
          }
        } catch (error) {
          //console.error(`Error checking optimization for image ${img.src}: ${error.message}`);
          logger.error(
            `${serviceName} Error checking optimization for image ${img.src}: ${error.message}`
          );

          nonOptimizedCount++;
          reasonsMap.other.push(imageUrl); // Categorize as "other"
          errorUrls.push(
            `Error checking optimization for image: ${img.src}. ${error.message}`
          );
        }
      }
      ...
      logger.error(
        `${serviceName} Error analyzing images using Puppeteer: ${error.message}`
      );
  ...
  async isImageOptimized(
    imageUrl: string
  ): Promise<{ optimized: boolean; reasons: string[] }> {
    try {
      ...
      logger.error(
        `${serviceName} Error checking optimization for image ${imageUrl}: ${error.message}`
      );
      return {
        optimized: false,
        reasons: [],
  ...
  async analyzeContentQuality(htmlContent: string) {
    ...
    const uniqueWordsPercentage =
      (new Set(textWords).size / textWords.length) * 100;
    ...
    if (uniqueWordsPercentage < 50) {
      recommendations += `The unique words percentage in your content is ${uniqueWordsPercentage.toFixed(
        2
      )}%, which indicates that your content may be repetitive. Try revising your content to introduce more variety in your language. `;
    }
  ...
  async analyzeSiteSpeed(url: string) {
    ...
    try {
      ...
      const loadTime =
        data.lighthouseResult.audits['speed-index'].numericValue / 1000; // Convert milliseconds to seconds

      let recommendations = '';

      if (loadTime > 5) {
        recommendations += `The page load time is ${loadTime.toFixed(
          2
        )} seconds, which is quite high. Users might experience noticeable delays. Consider optimizing images, reducing server response times, and leveraging browser caching to improve loading speed. `;
      } else if (loadTime > 3) {
        recommendations += `The page load time is ${loadTime.toFixed(
          2
        )} seconds, which is above the recommended 3 seconds. Try to streamline your page by minimizing the size of resources and improving server performance for a better user experience. `;
      } else {
        recommendations += `The page load time is ${loadTime.toFixed(
          2
        )} seconds, which is well within the recommended limits. `;
      }
      ...
      logger.error(
        `${serviceName} Error analyzing site speed: ${error.message}`
      );
  ...
  async analyzeMobileFriendliness(url: string, browser: puppeteer.Browser) {
    ...
    try {
      ...
      logger.error(
        `${serviceName} Error analyzing mobile-friendliness: ${error.message}`
      );
  ...
  async analyzeStructuredData(htmlContent: string) {
    ...
    if (count === 0) {
      ...
      recommendations += `Your site includes ${count} ${
        count > 1 ? 'structured data elements' : 'structured data element'
      }. `;
  ...
  async analyzeXmlSitemap(url: string) {
    try {
      ...
      logger.error(
        `${serviceName} Error fetching XML sitemap: ${error.message}`
      );
    ...
    try {
      const response = await axios.get(
        `https://www.googleapis.com/pagespeedonline/v5/runPagespeed?url=${encodeURIComponent(
          ...
          this.API_KEY
        }&category=performance&category=accessibility&category=best-practices&strategy=desktop`
      );
      const data = response.data;
      ...
      const getDiagnostics = (category: string) => {
        ...
        for (const auditKey in audits) {
          ...
          if (
            ...
            const descriptionWithoutBrackets = audit.description.replace(
              /\[.*?\]|\(.*?\)/g,
              ''
            );
            diagnostics.push({
      ...
      logger.error(
        `${serviceName} Error fetching Lighthouse data: ${error.message}`
      );
