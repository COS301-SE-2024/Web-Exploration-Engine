...
export class ScrapeReviewsService {
  async scrapeReviews(
    ...
    logger.info(
      serviceName,
      `Starting review scraping for URL: ${url}`,
      'url',
      url
    );
    ...
    if (!businessName) {
      logger.error(serviceName, 'Failed to extract business name from URL');
      return null;
    }
  ...
  private async scrapeReviewsViaGoogle(
    businessName: string,
    browser: puppeteer.Browser
  ): Promise<ReviewData> {
    ...
    try {
      ...
      logger.info(
        serviceName,
        `Navigated to Google search results for "${businessName} reviews`,
        'businessName',
        businessName
      );
      ...
      try {
        ...
        logger.error(
          serviceName,
          `Failed to scrape reviews from ${url}: ${error.message}`
        );
      }
      ...
      logger.error(
        serviceName,
        `Failed to perform Google search: ${error.message}`
      );
      return null;
    } finally {
      await browser.close();
  ...
  private async scrapeReviewsFromHelloPeter(
    page: puppeteer.Page
  ): Promise<ReviewData> {
    try {
      await page.waitForSelector('span.has-text-weight-bold', {
        timeout: 15000,
      });
      ...
      const {
        ...
        const elements = Array.from(
          document.querySelectorAll('div.perf-card__head')
        );
        const ratingElement = document.querySelector(
          'span.has-text-weight-bold'
        );
        const reviewCountElement = document.querySelectorAll(
          'span.has-text-weight-bold'
        )[1];
        const trustindexRatingElement = document.querySelector(
          'div.perf-card__head'
        );
        const npsElement = elements.find((el) =>
          el?.textContent?.includes('NPS')
        );
        const recommendationStatusElement = document.querySelector(
          'span.has-text-weight-bold.color-blue-v2'
        );

        const reviewNumbersElements = Array.from(
          document.querySelectorAll(
            'div.is-flex.falign-center.margin-bottom-10 span.fb__breakdown'
          )
        );
        const reviewNumbers = reviewNumbersElements.map((el) =>
        ...
        const reviewCount = reviewCountElement
          ? (reviewCountElement as HTMLElement).innerText.trim()
          : 'No review count found';
        const trustindexRating = trustindexRatingElement
          ? (trustindexRatingElement as HTMLElement).innerText.trim()
          : 'No Trustindex rating found';
        const nps = npsElement
          ? (npsElement as HTMLElement).innerText.trim()
          : 'No NPS found';
        const recommendationStatus = recommendationStatusElement
          ? (recommendationStatusElement as HTMLElement).innerText.trim()
          : 'No recommendation status found';
      ...
      throw new Error(
        `Failed to scrape reviews from Hello Peter: ${error.message}`
      );
    }
  }
  ...
  private extractBusinessNameFromUrl(url: string): string {
    try {
      ...
      logger.error(
        serviceName,
        `Failed to extract business name from URL: ${error.message}`,
        error.message
      );
      return '';
    }
