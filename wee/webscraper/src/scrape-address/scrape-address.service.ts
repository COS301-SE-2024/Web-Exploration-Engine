    ...
    try {
      ...
      const addresses = await page.evaluate(() => {
        const addressPattern =
          /\d+\s[\w\s]+(?:Ave|Avenue|Blvd|Boulevard|St|Street|Rd|Road|Dr|Drive|Lane|Way|Circle|Square|Pl|Place|Court|Crescent|Terrace|Park|Close|Mews|Row)\b(?:,?[\w\s]+){0,2}/g;
        const textContent = document.body.innerText;
        return Array.from(textContent.matchAll(addressPattern), (match) =>
          match[0].trim()
        );
      });
      ...
      const validAddresses = cleanAddresses.filter((address) => {
        ...
        const notBlacklisted = !blacklist.some((blacklistedWord) =>
          address.includes(blacklistedWord)
        );
      ...
      logger.error(
        `${serviceName} Failed to scrape addresses: ${error.message}`
      );
