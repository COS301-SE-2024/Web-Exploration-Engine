...
export class NewsScraperService {
  ...
  > {
    try {
      logger.info(
        serviceName,
        `Starting fetchNewsArticles for URL: ${url}`,
        'url',
        url
      );
      ...
      if (!businessName) {
        logger.error(
          `${serviceName} Failed to extract business name from URL: ${url}`
        );
      ...
      if (response.status !== 200) {
        logger.error(
          `${serviceName} Failed to fetch Google News RSS feed for ${businessName}`
        );
  ...
  private extractBusinessName(url: string): string | null {
    try {
      ...
      if (
        ...
        commonDomains.includes(filteredParts[filteredParts.length - 2])
      ) {
      ...
      logger.error(serviceName, ` Invalid URL provided: ${url}`, 'url', url);
      return null;
    }
  }
